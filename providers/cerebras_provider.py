...
CACHE_FAIL_PROMPT = {} 
MAX_LIST_RETRIES = 6 # The maximum number of retries for cerebras list translation
MAX_STRING_RETRIES = 3 # The maximum number of retries for cerebras string translation
...
# The CerebrasProvider class is a provider that uses the Cerebras API to translate text from one language to another via LLM, expect a high quality translation but it is very slow (100 examples every 6-7 minutes)
# The list translation is supported, the provider will return a list of translated text but Cerebras API for JSON is very slow and unreliable, so it is not recommended to use this provider for list translation
class CerebrasProvider(Provider):
    def __init__(self):
        try:
            ...
            raise KeyError("Please set the environment variable CEREBRAS_API_KEY by running `export CEREBRAS_API_KEY=<your_api_key>`, the API key can be obtained from https://cloud.cerebras.ai/platform, it is free to sign up and use the API.")

        self.translator = self.cerebras_client.chat.completions.create
    ...
    def _do_translate(self, input_data: Union[str, List[str]],
        ...
        try:
            ...
            print(f"\nCurrent Cerebras fail cache: {CACHE_FAIL_PROMPT}\n")
            raise e
